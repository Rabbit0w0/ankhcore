...
class SubscriptLifecycleProcessor @Inject private constructor(
  ...
  override fun process(
    ...
    val annotation = AnnotationUtil.create<SubscriptLifecycle>(annotationNode, ankhClassLoader)
    when (annotation.value) {
      PluginLifeCycle.LOAD -> ankhPluginContainer.onLoad(annotation.priority, callMethod)
      PluginLifeCycle.ENABLE -> ankhPluginContainer.onEnable(annotation.priority, callMethod)
      PluginLifeCycle.DISABLE -> ankhPluginContainer.onDisable(annotation.priority, callMethod)
    }
  }
}