...
import org.inksnow.ankh.core.common.dsl.getValue
import org.inksnow.ankh.core.common.dsl.logger
import org.inksnow.ankh.core.common.dsl.setValue
...
class AnkhWorldService @Inject private constructor(
            ...
            ?.let { ankhBlock ->
              syncActions.add {
                handleBlockSet(
                  ...
                  locationEmbedded = LocationEmbedded.warp(
  ...
  private fun runAsyncTick(
    location: LocationEmbedded,
    block: AsyncTickableBlock,
  ) {
    try {
  ...
  private fun onBlockBreakMonitor(event: BlockBreakEvent) {
    val locationEmbedded =
      LocationEmbedded.of(event.block.location)
    val chunkStorage = getWorldStorage(locationEmbedded.chunk().worldId()).getChunkStorage(locationEmbedded.chunk())
  ...
  class WorldStorage(
    ...
    val chunks =
      MapMaker().makeMap<WorldChunkEmbedded, ChunkStorage>()
  ...
  class ChunkStorage(
    ...
    val blockTickers =
      MapMaker().makeMap<LocationEmbedded, TickableBlock>()
    val asyncTickers =
      MapMaker().makeMap<LocationEmbedded, AsyncTickableBlock>()
    val blockMap =
      MapMaker().makeMap<LocationEmbedded, AnkhBlock>()
  }
}