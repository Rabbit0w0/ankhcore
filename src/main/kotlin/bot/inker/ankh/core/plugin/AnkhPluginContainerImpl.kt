...
import bot.inker.ankh.core.ioc.BridgerInjector
...
class AnkhPluginContainerImpl(
  ...
  override fun callInit(bukkitPlugin: AnkhBukkitPlugin) = ensureSuccess {
    ...
    if (pluginClass == AnkhCoreLoaderPlugin::class.java) {
      ...
      this.injector = (AnkhCoreLoaderPlugin.container as AnkhPluginContainerImpl).injector.createChildInjector(
        ...
        { binder ->
          ...
          binder.bind(AnkhBukkitPlugin::class.java).to(bukkitPlugin::class.java)
          binder.bind(bukkitPlugin::class.java as Class<AnkhBukkitPlugin>).toInstance(bukkitPlugin)
        },
      )
    }
    initListeners.call()
